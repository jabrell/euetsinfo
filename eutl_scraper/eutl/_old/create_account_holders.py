...
def create_account_holders(
    fn_transactions: Path, fn_manual_account_data: Path, dir_out: Path | None = None
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Load the transaction and manual account data, which are used for holder
    extraction.

    Args:
        fn_transactions (Path): Path to the transaction data CSV file.
        fn_manual_account_data (Path): Path to the manual account data Excel file.
        dir_out (Path | None): Optional directory to save the extracted account
            holders
    ...
    df_holder_trans = _get_holders_from_transactions(
        fn_transactions=fn_transactions
    ...
    if dir_out:
        df_link_accounts_holders.to_csv(
            dir_out / "eutl_link_accounts_holders.csv", index=False
        )
        df_holders.to_csv(dir_out / "eutl_account_holders.csv", index=False)
