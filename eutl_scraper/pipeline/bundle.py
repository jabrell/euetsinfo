...
A :class:`Bundle` is a named, **flat** list of :class:`Pipeline` instances. The
list is the bundle's content; running the bundle runs the pipelines in order.
A Bundle never contains another Bundle — bundles compose by *flattening*: a
coarser bundle's ``_build_pipelines`` inlines a finer bundle's ``.pipelines``
attribute at construction time. This keeps the runtime model simple (a bundle
is always a list of pipelines, never a tree) while still allowing the user to
run a finer-grained bundle on its own.
...
class Bundle(ABC):
    ...
    :meth:`_build_pipelines`, which returns the pipelines in the order they
    ...
    ``.pipelines`` attribute in ``_build_pipelines``. Composition lives at
    the factory level, not in the runtime type — the bundle still holds a
    flat ``list[Pipeline]``.
    ...
    def _build_pipelines(self) -> list[Pipeline]:
        ...
        Conditional inclusion (e.g. only add a pipeline when its API key is
        provided) belongs here, not in :meth:`run`. To compose from finer
